...
async def get_announcements(request: Request) -> JSONResponse:
    ...
    **Returns:**
    ...
    announcements = [AnnouncementResponseModel.to_response(
        announcement) for announcement in session.query(AnnouncementDbModel).all()]
...
async def get_announcements_for_channel(request: Request, channel_id: int) -> JSONResponse:
    """
    ...
    **Returns:**
    ...
    announcements = [AnnouncementResponseModel.to_response(announcement) for announcement in session.query(
        AnnouncementDbModel).filter_by(channel_id=channel_id).all()]
...
async def get_latest_announcement(request: Request) -> JSONResponse:
    ...
    **Returns:**
    ...
    announcement = session.query(AnnouncementDbModel).order_by(
        AnnouncementDbModel.id.desc()).first()
...
async def get_latest_announcement_for_channel(request: Request, channel_id: int) -> JSONResponse:
    """
    ...
    **Returns:**
    ...
    announcement = session.query(AnnouncementDbModel).filter_by(
        channel_id=channel_id).order_by(AnnouncementDbModel.id.desc()).first()
...
async def post_announcement(request: Request, channel_id: int) -> JSONResponse:
    ...
    **Args:**
    ...
    attachments = list(map(lambda url: AttachmentDbModel(attachment_url=url),
                       content['attachments'])) if content and 'attachments' in content else []
