...
export async function getRelease(env: Env, prerelease: boolean) {
  ...
  const release = await backend.release(organization, manager.repo, prerelease);
  const asset = release.assets.find((item) => manager.assetRegex.test(item.name));
...
export async function getDownloadersRelease(env: Env, prerelease: boolean) {
  ...
  const release = await backend.release(organization, manager.downloadersRepo, prerelease);
  const asset = release.assets.find((item) => manager.downloadersAssetRegex.test(item.name));
