...
app.openapi(
  createRoute({
    ...
    responses: {
      200: {
        content: { "application/json": { schema: AnnouncementsResponseSchema } },
...
app.openapi(
  createRoute({
    ...
    request: {
      body: { content: { "application/json": { schema: CreateAnnouncementBodySchema } } },
    },
    responses: {
      201: {
...
app.openapi(
  ...
  async (c) => {
    ...
    const body = c.req.valid("json");
    const result = await announcementsService.updateAnnouncement(c.env, id, body);
    if (!result) {
